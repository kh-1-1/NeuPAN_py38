...
def shift_down(input, size=1):
    """
    Shifts given tensor down by `size` rows:
    1. Pad the top of the tensor with `size` rows of zeros
    2. Remove the bottom `size` rows
    :param input: Tensor. Tensor to be shifted
    :param size: Int. Number of rows to shift
    :return: Tensor. Shifted input
    """
...
def shift_right(input, size=1):
    """
    Shifts given tensor right by `size` columns:
    1. Pad the left of the tensor with `size` columns of zeros
    2. Remove the rightmost `size` columns
    :param input: Tensor. Tensor to be shifted
    :param size: Int. Number of columns to shift
    :return: Tensor. Shifted input
    """
...
class PixelSNAIL(nn.Module):
    ...
    def ids2vectors(self, ids):
        """
        Transforms given tensor of ids to tensor of one-hot encodings
        """
        vectors = (
            F.one_hot(ids.clone(), self.n_class).permute(0, 3, 1, 2).type_as(self.background)
        )
    ...
    def forward(self, input, condition=None, cache=None):
        ...
        ## Following code is only relevant when doing VQ-VAE cascades
        # if condition is not None:
